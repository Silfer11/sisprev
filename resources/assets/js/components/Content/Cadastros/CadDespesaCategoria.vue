<template>
  <v-dialog v-model="dialog" persistent max-width="500px">
      <v-btn slot="activator"><v-icon>add</v-icon>Cadastrar Categoria de Despesa</v-btn>
      <v-card>
        <v-card-title>
          <span class="headline">Cadastrar Categoria de Despesa</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-form v-model="valid" ref="form" lazy-validation>
              <v-layout wrap>
                <v-flex xs12>
                  <v-text-field label="Categoria" v-model="categoria" required></v-text-field>
                </v-flex>
              </v-layout>
            </v-form>
          </v-container>
